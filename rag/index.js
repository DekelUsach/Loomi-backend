...
const memoryByStory = new Map();
let lastAssignedStoryId = -1;
...
export async function deleteStory(storyId) {
  ...
  try {
    ...
    if (kept.length === (rows?.length || 0)) return true;
...
export async function releaseMemoryForStory(storyId) {
  await ensureMemoryLoaded();
  memoryByStory.delete(String(storyId));
  await saveMemoryCache();
  return true;
...
export async function storyExists(storyId) {
  try {
    const mem = memoryByStory.get(storyId);
